...
import { Loader2, AlertCircle, Megaphone, Calendar, User, ChevronRight, History, Tag, GitCommit } from 'lucide-react';
...
const NewsClient = ({ initialTab = 'news' }: { initialTab?: 'news' | 'changelog' }) => {
  const pathname = usePathname();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<'news' | 'changelog'>(initialTab);
  
  // Sync tab if props change (e.g. navigation)
  useEffect(() => {
              ...
              {[
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    router.push(tab.id === 'news' ? '/news' : '/changelog');
                  }}
                  className={clsx(
